...
以下のコードは、Dat01（赤色）が勉強していないグループの試験得点、Dat02（青色）が勉強しているグループの試験得点を各グループ100人を乱数を用いて作成し、ヒストグラムで表示した結果です。
...
先ほど2つのデータ群について平均値や中央値を求めましたが、これらのデータだけを比較し、高い低いだけを論じることは本当に適切でしょうか。残念ながら、科学の世界では単にこれらを比較するだけでは、その結果は信用のあるものとして受け入れられません。平均値同士の差がどの程度信頼できるのか、その結果も添えてあげる必要があります。平均値の差に対する信頼性として、多くの場合、t検定による$p$値を用いることが普通です。
