...
空間相関とは**近い地域ほど似た特徴を持つ**という空間の基本原理である。
これを扱うためには、**近さ**をルーク型・クイーン型・k近傍などで明確に定義する必要がある。
近さの関係は近接行列として数値化され、空間統計で重要な役割を果たす。
行を正規化した近接行列は、地域ごとの隣接数の違いを調整し、比較を公平にする。(次回の演習で活躍します)
プログラムを用いて、近さの定義から近接行列の生成・可視化まで一連の分析を実践的に行った。
