...
それは棚に置くとして、全世界測位システム(Global Positioning System; GPS)に代表される
全地球衛星航法システム(Global Navigation Satellite System; GNSS)の発達により、人・物の位置の補足・追跡が容易になりました。

## 空間データの可視化例 ~東京都デジタルツインプロジェクト~
...
古いデータ(2000年辺りから前)を取り扱う際にはこちらのデータを使用することがありますが、現在では世界測地系(WGS84)を使用します。
WGS84はよく使われる測地系ですが、位置座標が緯度軽度で与えられているため、適切な距離計算が困難です。具体的には第12回で紹介します。
...
次に、[このファイル](../materials/03/FG-GML-533944-DEM5A-20350620.zip)をダウンロードしてください。
