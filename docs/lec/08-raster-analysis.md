...
- [第8回 ラスタデータの分析](#第8回-ラスタデータの分析)
  - [この講義の目次](#この講義の目次)
  - [ラスタデータとは（復習）](#ラスタデータとは復習)
  - [本講義の目的](#本講義の目的)
  - [はじめに](#はじめに)
  - [数値標高モデルの視覚的分析](#数値標高モデルの視覚的分析)
  - [ラスターデータとDEM](#ラスターデータとdem)
  - [陰影図の作成](#陰影図の作成)
  - [鳥観図の作成](#鳥観図の作成)
  - [地形断面図の作成](#地形断面図の作成)
  - [今回のまとめ](#今回のまとめ)
...
次にDEMデータを用いて陰影図を作成してみましょう。

- 「ラスタ」ー「ラスタ解析」ー「陰影図(hillshade)」を選択
- そのまま「実行」（tifデータの保存は任意）
  - ![picture](../materials/08/figure_13.webp)

このような図が作成されました
![picture](../materials/08/figure_14.webp)

これを先程色調整したレイヤの下に配置すると、立体感のある図が作成されます。
![picture](../materials/08/figure_15.webp)

## 鳥観図の作成

*マシンスペックが必要になるので注意してください

- 「ビュー」ー「新しい3Dマップビュー」を選択し、「3Dビューの設定（スパナ）」アイコンを選択

このような3D地図ができます。視点変更はマウスがあると便利です。
![picture](../materials/08/figure_16.webp)

この図は、渋谷から明治神宮方面を見た視点になります。
原宿周辺は、谷の地形を活用して街が形成されたことがよくわかるかと思います。
また、表参道はなだらかな傾斜に形成されていることが読み取れます。
この地図の右上にある明治神宮はこの地域としては高い場所にあり、その区画はきれいに整備されていることがわかりますね。
